...
The cache section of the config file holds information about where to store a cache for large data sets that change infrequently (such as context variables for custom query). This helps speed up your integration by removing the time needed to speak to the LoanPro server. By default, the cache will be stored in a cache.json file in the working directory with a 3 day expiration. However, for many installations this may not work (eg. there may not be write permissions to the working directory). To change cache settings, you can use the following key-value pairs in the cache section:
...
### Setting Configuration with Code
...
* $cacheFile - The file to use for caching (if not specified will not change cache file settings)
* $cacheExpr - The string for the DateInterval of how long the cache will be valid (if not specified will not change cache expiration settings)
...
* <a href="http://api.loanprosoftware.com/sdks/php/doxygen">Elasticsearch Docs</a>
